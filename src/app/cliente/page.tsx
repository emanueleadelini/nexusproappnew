      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-slate-500 font-bold uppercase tracking-wider text-xs">Area Riservata &bull; AD next lab Hub</p>
          </div>
        </div>
            ...
            <TabsContent value="feed" className="space-y-12 outline-none">
              <div className="max-w-lg mx-auto space-y-12">
                <div className="flex justify-center">
                  <Badge variant="outline" className="bg-white text-indigo-600 border-indigo-100 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider shadow-sm">
            ...
            <TabsContent value="calendar">
              <div className="glass-card p-8 rounded-[2.5rem] border-none shadow-sm bg-white">
                <div className="mb-6 flex items-center justify-between">
                  ...
                  <Badge variant="outline" className="border-indigo-100 text-indigo-600 bg-indigo-50/50 uppercase font-black text-[10px] px-3 py-1 tracking-wider">SOLO CONSULTAZIONE</Badge>
                </div>
                  ...
                  <Card className="glass-card rounded-[2rem] border-none shadow-sm overflow-hidden bg-white">
                    ...
                    <CardContent className="p-6 space-y-4">
                        ...
                        <div key={m.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 group">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="text-[10px] text-slate-500 font-bold uppercase">Firmato il {m.creato_il?.toDate().toLocaleDateString('it-IT')}</p>
                            </div>
                          </div>
                      ...
                      {materials?.filter(m => m.destinazione === 'contratto').length === 0 && (
                        <p className="text-center py-8 text-xs font-bold text-slate-500 uppercase italic">Nessun contratto archiviato.</p>
                      )}
                    </CardContent>
                  </Card>
                )}
                ...
                {clientData?.include_visual_identity && (
                  <Card className="glass-card rounded-[2rem] border-none shadow-sm overflow-hidden bg-white">
                    <CardHeader className="bg-indigo-50 p-6">
                      <CardTitle className="text-indigo-600 text-xs font-black uppercase tracking-wider flex items-center gap-2">
                    ...
                    <CardContent className="p-6 space-y-4">
                      {materials?.filter(m => m.destinazione === 'visual_identity').map(m => (
                        <div key={m.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 group">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              <p className="text-xs font-bold text-slate-900 truncate">{m.nome_file}</p>
                              <p className="text-[10px] text-slate-500 font-bold uppercase">{m.creato_il?.toDate().toLocaleDateString('it-IT')}</p>
                            </div>
                          </div>
                      ...
                      {materials?.filter(m => m.destinazione === 'visual_identity').length === 0 && (
                        <p className="text-center py-8 text-xs font-bold text-slate-500 uppercase italic">Asset in fase di caricamento.</p>
                      )}
                    </CardContent>
                  </Card>
                )}
                      ...
                      {['brochure', 'volantino', 'bigliettini', 'gadget', '6x3', '3x6', 'altro'].map(type => {
                        ...
                        return (
                          <div key={type} className="space-y-4">
                            <h5 className="text-xs font-black uppercase tracking-wider text-slate-500 border-b border-slate-100 pb-2">{type}</h5>
                            <div className="space-y-3">
                              ...
                              {(!typeMaterials || typeMaterials.length === 0) && (
                                <p className="text-[10px] font-bold text-slate-400 uppercase italic">Nessun asset caricato</p>
                              )}
                            </div>
                          </div>
                        );
          ...
          <Card className="glass-card border-none rounded-[2.5rem] overflow-hidden shadow-sm bg-white">
            ...
            <CardContent className="p-8 space-y-8">
              <div className="text-center p-6 bg-slate-50 rounded-3xl border border-slate-100">
                <span className="text-xs font-black text-slate-500 uppercase tracking-wider">Post Residui</span>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-end"><span className="text-xs font-black text-slate-500 uppercase">Utilizzo</span><span className="text-sm font-black text-slate-900">{clientData?.post_usati} / {clientData?.post_totali}</span></div>
              ...
              <div className="pt-4 space-y-4">
                <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-bold uppercase">Servizio attivo al 100%</p>
                  </div>
                </div>
      ...
      <Dialog open={!!selectedPost} onOpenChange={() => setSelectedPost(null)}>
        <DialogContent className="bg-white border-slate-200 text-slate-900 rounded-3xl max-w-sm sm:max-w-md shadow-2xl">
          ...
          <div className="space-y-6 py-4">
            <div className="space-y-2">
              <label className="text-xs font-black uppercase text-slate-500 tracking-wider">Le tue note</label>
